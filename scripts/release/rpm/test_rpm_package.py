...
# From Fedora 36, when using `pip install --prefix`with root, the package is installed into `{prefix}/local/lib`
# See https://docs.fedoraproject.org/en-US/fedora/latest/release-notes/developers/Development_Python/
pytest_base_cmd = f'PYTHONPATH=/usr/lib64/az/lib/{python_version}/site-packages:/usr/lib64/az/local/lib/{python_version}/site-packages python3 -m pytest -x -v --boxed -p no:warnings --log-level=WARN'
