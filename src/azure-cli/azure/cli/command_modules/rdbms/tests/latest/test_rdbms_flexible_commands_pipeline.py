...
class FlexibleServerVnetServerMgmtScenarioTest(RdbmsScenarioTest):
    ...
    def _test_flexible_server_vnet_server_create(self, database_engine, resource_group, server):
        vnet = 'testvnet'
        subnet = 'testsubnet'
        self.cmd('{} flexible-server create -g {} -n {} -l {} --vnet {} --subnet {} --yes'
                 .format(database_engine, resource_group, server, self.location, vnet, subnet))
    ...
    def _test_flexible_server_vnet_ha_server_create(self, database_engine, resource_group, server):
        vnet = 'testvnet2'
        subnet = 'testsubnet2'
        self.cmd('{} flexible-server create -g {} -n {} -l {} --vnet {} --subnet {} --tier GeneralPurpose --sku-name Standard_D2s_v3 --high-availability Enabled --yes'
                 .format(database_engine, resource_group, server, self.location, vnet, subnet))
